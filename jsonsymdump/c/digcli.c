...
void *DIGCLIENT DIGCliRealloc( void *p, size_t amount )
{
    ...
    printf(( "DIGCliRealloc: p=%p amount=%3x -> %p\n", p, (unsigned)amount, ptr ));
    return( ptr );
}

void DIGCLIENT DIGCliFree( void *p )
{
    printf(( "DIGCliFree: p=%p\n", p ));
    free( p );
}
...
dig_fhandle DIGCLIENT DIGCliOpen( char const *name, dig_open mode )
{
    ...
    printf(( "DIGCliOpen: name=%p:{%s} mode=%#x\n", name, name, mode ));
...
unsigned long DIGCLIENT DIGCliSeek( dig_fhandle h, unsigned long p, dig_seek k )
{
    ...
    switch( k ) {
        ...
        default:
            printf(( "DIGCliSeek: h=%d p=%ld k=%d -> -1\n", h, p, k ));
            return( -1 );
    }

    off = lseek( h, p, whence );
    printf(( "DIGCliSeek: h=%d p=%ld k=%d -> %ld\n", h, p, k, off ));
    return( off );
}
...
unsigned DIGCLIENT DIGCliRead( dig_fhandle h, void *b , unsigned s )
{
    ...
    printf(( "DIGCliRead: h=%d b=%p s=%d -> %d\n", h, b, s, rc ));
    return( rc );
}
...
unsigned DIGCLIENT DIGCliWrite( dig_fhandle h, const void *b, unsigned s )
{
    ...
    printf(( "DIGCliWrite: h=%d b=%p s=%d -> %d\n", h, b, s, rc ));
    return( rc );
}

void DIGCLIENT DIGCliClose( dig_fhandle h )
{
    printf(( "DIGCliClose: h=%d\n", h ));
    if( close( h ) )
        printf(( "DIGCliClose: h=%d failed!!\n", h ));
}

void DIGCLIENT DIGCliRemove( char const *name, dig_open mode )
{
    printf(( "DIGCliRemove: name=%p:{%s} mode=%#x\n", name, name, mode ));
    unlink( name );
    mode = mode;
}
